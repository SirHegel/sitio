...
const RESERVED_SLUGS = new Set(["oro-perimetros"]);
...
export function validateManagedSlug(value) {
  const slug = validateSlug(value);
  if (RESERVED_SLUGS.has(slug)) {
    throw new HttpError(409, "slug_reservado", "La ruta pertenece a un estudio especial y no admite escritura desde el CMS.");
  }
  return slug;
}
...
export function validateArticle(
  input,
  { requireSlug = true, maxBodyCharacters = 60_000, maxBodyBytes = 80_000 } = {},
) {
  ...
  const cuerpo = cleanText(input.cuerpo, "El cuerpo", maxBodyCharacters);
  if (Buffer.byteLength(cuerpo, "utf8") > maxBodyBytes || RAW_HTML.test(cuerpo) || DANGEROUS_URI.test(cuerpo)) {
...
export function parseArticleMarkdown(
  markdown,
  { slug, sha, maxBodyCharacters = 60_000, maxBodyBytes = 80_000 } = {},
) {
  ...
  const parsed = validateArticle(
    { ...meta, slug: validateSlug(slug), sha, cuerpo: normalized.slice(end + 5).trim() },
    { maxBodyCharacters, maxBodyBytes },
  );
  return parsed;
}
...
export async function readArticle(slug, env = process.env, fetchImpl) {
  const safeSlug = validateManagedSlug(slug);
  try {
  ...
  try {
    ...
    for (let index = 0; index < files.length; index += 8) {
      ...
      const parsed = await Promise.all(batch.map(async (file) => {
        ...
        if (!SLUG.test(slug) || RESERVED_SLUGS.has(slug)) return null;
...
export async function saveArticle(input, env = process.env, fetchImpl) {
  ...
  validateManagedSlug(article.slug);
