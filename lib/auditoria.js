...
const PUBLIC_ROUTES = new Set([
  "/", "/academico/", "/proyectos/", "/blog/", "/actividad/", "/hoja-de-vida/",
...
const ATTRIBUTION_CHANNELS = new Set([
  "whatsapp", "telegram", "linkedin", "github", "facebook", "instagram",
  "x", "email", "qr", "sms", "signal",
]);
const REFERRER_CHANNELS = new Map([
  ["t.me", "telegram"], ["telegram.me", "telegram"],
  ["web.telegram.org", "telegram"], ["l.facebook.com", "facebook"],
  ["facebook.com", "facebook"], ["l.instagram.com", "instagram"],
  ["instagram.com", "instagram"], ["linkedin.com", "linkedin"],
  ["lnkd.in", "linkedin"], ["t.co", "x"], ["x.com", "x"],
  ["github.com", "github"],
]);
...
export function parseUserAgent(raw) {
  ...
  else if (/Android/i.test(ua) && dispositivo === "Tableta") familia = "Tableta Android";
  else if (/Android/i.test(ua)) familia = "Teléfono Android";
  else if (/Windows NT/i.test(ua)) familia = "PC Windows";
  else if (/Macintosh|Mac OS X/i.test(ua)) familia = "Mac";
  else if (/CrOS/i.test(ua)) familia = "Chromebook";
  else if (/Linux/i.test(ua)) familia = "PC Linux";
  return { dispositivo, familia, sistema, navegador };
}

function channelFromReferrer(referrer) {
  if (!referrer || ["Directo", "No disponible"].includes(referrer) || referrer.startsWith("Interno:")) return "";
  for (const [domain, channel] of REFERRER_CHANNELS) {
    if (referrer === domain || referrer.endsWith(`.${domain}`)) return channel;
  }
  return "";
}
...
function safeCampaign(value) {
  ...
  return /^[a-z0-9][a-z0-9_-]{0,31}$/.test(candidate) ? candidate : "";
}
...
export function validateVisitInput(input, origin = "https://sitio.invalid") {
  ...
  try {
    ...
    const via = safeCampaign(url.searchParams.get("via"));
    const utmSource = safeCampaign(url.searchParams.get("utm_source"));
    campana = ATTRIBUTION_CHANNELS.has(via)
      ? via
      : ATTRIBUTION_CHANNELS.has(utmSource) ? utmSource : "";
    const campaign = String(url.searchParams.get("utm_campaign") || "").toLowerCase();
  ...
  return { path, referrer, campana: campana || channelFromReferrer(referrer), enlace };
}
...
function cleanStoredEvent(item) {
  ...
  return {
    ...
    enlace: /^s-[a-f0-9]{12}$/.test(String(item.enlace || "")) ? item.enlace : "Sin identificador",
